...
document.addEventListener(
	...
	function(){
		...
		var gallerys = document.querySelectorAll( '.product-gallery' );

		gallerys.forEach(
			function (gallery) {  

				var	flickity_carousels = gallery.querySelector( '.flickity-carousels' ),
					productThumbnails = gallery.querySelector( '.product-thumbnail-image-list' ), //document.getElementById( 'product-thumbnail-images' )
				...
				if ( productThumbnails ) {
					...
					imageCarousel = new Flickity( flickity_carousels, options );

					calculateThumbnailTotalWidth();

					if ( gallery ) {
						if ( window.matchMedia( '( max-width: 767px )' ).matches ) {
							thumbCarousel = new Flickity( productThumbnails, thumbOptions );
						} else {
				...
				function changeImageCarouselButtonIcon() {
					var imageNextBtn = gallery.querySelector( '.flickity-button.next' );
					var imagePrevBtn = gallery.querySelector( '.flickity-button.previous' );
				...
				function addThumbButtons() {
					...
					var thumbButtons = productThumbnails.querySelectorAll( '.thumb-btn' );
				...
				function woostifyGalleryCarouselMobile() {
					var mobileGallery = gallery.querySelector( '.has-gallery-list-layout .product-gallery.has-product-thumbnails' );
				...
				function updateGallery( data, reset, variationId ) {
					...
					if ( images && gallery.querySelector( '.product-images' ) ) {
						// document.querySelector( '.product-images' ).querySelector( '#product-images' ).innerHTML = images;
						gallery.querySelector( '.product-images' ).querySelector( '.flickity-carousels' ).innerHTML = images;
					}

					if ( gallery.querySelector( '.product-thumbnail-images' ) ) {
						if ( '' !== thumbnails ) {
							var productThumbnailsWrapper = gallery.querySelector( '.product-thumbnail-images' ).querySelector( '.product-thumbnail-image-list' );
							...
							gallery.querySelector( '.product-thumbnail-images' ).appendChild( productThumbnailsWrapper ).innerHTML = thumbnails;

							if ( gallery ) {
								gallery.classList.add( 'has-product-thumbnails' );
							}
						} else {
							gallery.querySelector( '.product-thumbnail-images' ).innerHTML = '';
						}
					}

					// Re-init slider.
					if ( ! noSliderLayout ) {
						productThumbnails = document.getElementById( 'product-thumbnail-images' ); console.log(productThumbnails);
						if ( productThumbnails ) {
							options.on = {
								ready: function() {
									changeImageCarouselButtonIcon();
							...
							function( e, variation ) {
								...
								if ( 'undefined' !== typeof( woostify_variation_gallery ) && woostify_variation_gallery.length ) {
									...
									if( variation.variation_gallery_images || 0 ){
										var thumbs = gallery.querySelector( '.product-thumbnail-images' );
					...
					jQuery( '.reset_variations' ).on(
						...
						function(){
							if ( 'undefined' !== typeof( woostify_default_gallery ) && woostify_default_gallery.length ) {
								...
								if( gallery.querySelector( '.product-thumbnail-images' ) ) {
									gallery.querySelector( '.product-thumbnail-images').classList.remove( 'variation-gallery' );
								}
							}
							...
							if ( document.body.classList.contains( 'elementor-editor-active' ) || document.body.classList.contains( 'elementor-editor-preview' ) ) {
								if ( ! gallery.getElementById( 'product-thumbnail-images' ) ) {
					...
					onElementorLoaded(
						function() {
							window.elementorFrontend.hooks.addAction(
								...
								function() {
									if ( gallery.querySelector( '.product-thumbnail-image-list' ) ) {
										renderSlider( flickity_carousels, options );
										if ( window.matchMedia( '( min-width: 768px )' ).matches && gallery && gallery.classList.contains( 'vertical-style' ) ) {
											calculateVerticalSliderHeight();
										}else{
											renderSlider( productThumbnails, thumbOptions );
