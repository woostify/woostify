jQuery(document).ready(function ($) {
    ...
    var VideoGallery = {
        ...
        bindEvents: function () {
            ...
            // Clear video data when an image is removed from the gallery
            $('#product_images_container').on('click', 'a.delete', function () {
                var attachmentId = $(this).closest('li.image').attr('data-attachment_id');
                self.clearVideoData(attachmentId);
            });

            // Clear video data when main product image is removed
            $('#postimagediv').on('click', '#remove-post-thumbnail', function () {
                var btn = $(this).closest('.inside').find('.woostify-add-video-btn');
                if (btn.length) {
        ...
        clearVideoData: function (attachmentId) {
            if (!attachmentId) return;

            // Remove from local map so if it's added back without reload, it has no video
            var videoMap = woostify_product_video_gallery.video_exists || {};
            if (videoMap[attachmentId]) {
                delete videoMap[attachmentId];
                woostify_product_video_gallery.video_exists = videoMap;

                // Submit AJAX to clear backend data
                $.ajax({
                    url: ajaxurl,
                    type: 'POST',
                    data: {
                        action: 'woostify_clear_attachment_video_data',
                        nonce: woostify_product_video_gallery.nonce,
